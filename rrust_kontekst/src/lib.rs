...
//use log::info;
use syn::{self, Error, meta::ParseNestedMeta, parse_macro_input};
...
fn generate_handler_function(config: &McpConfig, fn_name: &syn::Ident) -> proc_macro2::TokenStream {
...
fn generate_registration(config: &McpConfig, fn_name: &syn::Ident) -> proc_macro2::TokenStream {
    let mcp_handler_name = syn::Ident::new(&format!("{}_mcp_handler", fn_name), fn_name.span());
